...
class barang{
	...
	public function tambah($nama_brg,$gambar,$jenis,$suplier,$harga_modal,$harga_jual,$jumlah,$deskripsi)
	{
		try {
			$stmt = $this->db->prepare("INSERT INTO tb_barang(nama_brg,gambar,jenis,suplier,harga_modal,harga_jual,jumlah,deskripsi) VALUES (:nama_brg, :gambar, :jenis, :suplier, :harga_modal, :harga_jual, :jumlah, :deskripsi)");
	...
	public function update($id,$nama_brg,$jenis,$suplier,$harga_modal,$harga_jual,$jumlah,$sisa)
	{
		...
		if ($sisa >=1) { /*jika sisa lebih besar atau sama dengan 1 maka yg di update hanya data jumlah*/
			...
			try {
			$stmt = $this->db->prepare("UPDATE tb_barang SET nama_brg=:nama_brg, gambar=:gambar, jenis=:jenis, suplier=:suplier, harga_modal=:harga_modal, harga_jual=:harga_jual, jumlah=:jumlah WHERE id_barang=:id");
	...
	public function updateku($id,$nama_brg,$gambar,$jenis,$suplier,$harga_modal,$harga_jual,$jumlah,        $deskripsi,$sisa)
	{
		// $sisa = true;
		if ($sisa >=1) { /*jika sisa lebih besar atau sama dengan 1 maka yg di update hanya data jumlah*/
			try {
			...
			try {
			$stmt = $this->db->prepare("UPDATE tb_barang SET nama_brg=:nama_brg, gambar=:gambar, jenis=:jenis, suplier=:suplier, harga_modal=:harga_modal, harga_jual=:harga_jual, jumlah=:jumlah, deskripsi=:deskripsi WHERE id_barang=:id");
			$stmt->bindparam(":id",$id);
			$stmt->bindparam(":nama_brg",$nama_brg);
			$stmt->bindparam(":gambar",$gambar);
			$stmt->bindparam(":jenis",$jenis);
	...
	public function view_data_barang($query,$table)
	{
		...
		$no = 1;
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				<td>
					<a href="detail.php?id=<?php print($row['id_barang']); ?>"><span class="btn btn-info">Detail</span></a>
	    			<a href="edit.php?edit_id=<?php print($row['id_barang']); ?>"><span class="btn btn-warning">Edit</span></a>
	    			<a href="hapus.php?hapus_id=<?php print($row['id_barang']); ?>"><span class="btn btn-danger">Hapus</span></a>
				</td>
				</tr>
				<?php
			}
		 ...
		 $no = 1;
			while ($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
				...
				<td><?php print($row['jumlah']); ?></td>
				<td>
	    <a href="terjual_edit.php?edit_id=<?php print($row['id_terjual']); ?>"><span class="btn btn-warning">Edit</span></a>
	    <a href="terjual_hapus.php?hapus_id=<?php print($row['id_terjual']); ?>"><span class="btn btn-danger">Hapus</span></a>
				</td>
				</tr>
				<?php
			}
	...
	public function pesan($query)
	{
		...
		while ($row=$stmt->fetch(PDO::FETCH_ASSOC)) {
			if (isset($row['jumlah'])<=5) {
				...
				<?php
				echo "<div style='padding:5px' class='alert alert-warning'><span class='glyphicon glyphicon-info-sign'></span> Data <a style='color: red'>".$row['nama_brg']."</a> gagal di input!! </div>";
			}elseif($row['jumlah']>=6) {
